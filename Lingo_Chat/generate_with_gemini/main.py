...
def main(system_prompt:str,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(gemini_api_key_list)) as executor:
        futures = [executor.submit(_process_data, idx, _data) for idx, _data in enumerate(koalpaca_orbit) if idx >= start_idx]
        for future in tqdm(concurrent.futures.as_completed(futures), total=(len(koalpaca_orbit)-start_idx)):
            future.result()
...
def gen_new_turn(model_name: str,
    ...
    async def _run(_user_input, _induce_prompt, gemini_api_key, temperature) -> str:
        chat_history = [
            {"role": "user", 
                "parts": [{
                    "text": _user_input
    ...
    gen_question = asyncio.run(_run(_user_input, question_induce_prompt, gemini_api_key, 0.5))
    _new_user_input = _user_input.split('\n\n\n')[0]+'\nUser: '+gen_question+'\n\n\n'
    gen_answer = asyncio.run(_run(_new_user_input, answer_induce_prompt, gemini_api_key, 0))
...
if __name__ == '__main__':
    ...
    load_dotenv()
    gemini_api_key_list = [os.getenv(f"{i}_GEMINI_API_KEY") for i in ["LE"]]    # ["LE", "JH", "JY", "YH"]
    start_idx = 1374

    main(system_prompt=system_prompt,
         model_name="geminipro1.0",
         gemini_api_key_list=gemini_api_key_list,
         data_save_path='data/multiturn_data_0626_2.jsonl',
         target_turn=3,
         start_idx=start_idx
         )